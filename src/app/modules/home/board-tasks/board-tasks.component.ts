...
import { BroadTasksService } from './board-tasks.service';
import { Board, Card, List } from './models/board-task.models';
...
export class BoardTasksComponent implements OnInit {
  ...
  constructor(
      ...
      private _broadTasksService: BroadTasksService
  )
  {
  }
  ...
  addList(title: string): void
  {
      ...
      // Save the list
      this._broadTasksService.createList(list).subscribe();
  }
  ...
  updateListTitle(event: any, list: List): void
  {
      ...
      // Update the list
      this._broadTasksService.updateList(list).subscribe();
  }
  ...
  deleteList(id): void
  {
      ...
      confirmation.afterClosed().subscribe((result) => {
          ...
          if ( result === 'confirmed' )
          {
              ...
              // Delete the list
              this._broadTasksService.deleteList(id).subscribe();
          }
      });
  }
  ...
  addCard(list: List, title: string): void
  {
      ...
      // Save the card
      this._broadTasksService.createCard(card).subscribe();
  }
  ...
  listDropped(event: CdkDragDrop<List[]>): void
  {
      ...
      // Update the lists
      this._broadTasksService.updateLists(updated).subscribe();
  }
  ...
  cardDropped(event: CdkDragDrop<Card[]>): void
  {
      ...
      // Update the cards
      this._broadTasksService.updateCards(updated).subscribe();
  }
