          ...
          this.appService.getHabilitation(user, this.moduleIdentifier)
            .then(habilitation => {
              if (habilitation.unauthorized()) {
                ...
                this.router.navigateByUrl('/tickets/admin-tickets');
                resolve();
              } else if (this.routePath.indexOf('admin-tickets') !== -1 && !habilitation.isAdmin()) {
                this.router.navigateByUrl('tickets/employee-tickets');
                resolve();
              }
              else {
                //localhost:4209/tickets/admin-tickets
                console.log('##### else Z, route config', route.routeConfig)
                resolve();
